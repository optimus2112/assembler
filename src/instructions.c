...
int get_reg_from_label(char* label) {
    ...
    reg = get_reg(tmp);
    if (reg == 0 && strcmp(tmp, "r0"))  return NONE;
    return reg;
}
...
int get_ins_size(Instruction* instr) {
    int size = 2;
    if (get_arg_type(instr->first_argument) == LABEL || get_arg_type(instr->second_argument) == LABEL) size = 4;
    if (get_arg_type(instr->first_argument) == NUMBER || get_arg_type(instr->second_argument) == NUMBER) size++;
    if (get_arg_type(instr->first_argument) == LABEL && get_arg_type(instr->second_argument) == LABEL) size = 6;
    if (instr->name == STOP || instr->name == RTS) size = 1;
    return size;
}
...
int build_label_only_ins(Instruction* instr) {
    ...
    if (l1->EE == EXTERN) add_to_extern(l1->name, get_ic() + 2);
...
int build_label_number_ins(Instruction* instr) {
    ...
    if (get_arg_type(instr->first_argument) == NUMBER) { 
        ...
        if (!number_check(instr->first_argument)) return BUILD_FAILED;
        instr->bin[2] = get_number_arg(instr->first_argument);
        if (l1->EE == EXTERN) add_to_extern(l1->name, get_ic() + 3);
        ...
        instr->bin[2] = l1->base_address;
        instr->bin[3] = l1->offset;
        if(!number_check(instr->second_argument)) return BUILD_FAILED;
...
int build_label_label_ins(Instruction* instr) {
    ...
    if (l1->EE == EXTERN) add_to_extern(l1->name, get_ic() + 2);/*Check if the labels are external*/
    if (l2->EE == EXTERN) add_to_extern(l2->name, get_ic() + 4);
    instr->bin[2] = l1->base_address;
    instr->bin[3] = l1->offset;
...
int build_instruction(char* line) {
    ...
    int size;
    int ptr;
    if (!ins_structure_check(line)) return BUILD_FAILED; 
    strcpy(cp, line);
    fill_instruction(&instr, cp); /*Passing copy of line since fill_instruction modifies the input*/
    if (!check_ins_args(&instr)) return BUILD_FAILED;
